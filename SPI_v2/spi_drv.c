...
int spi_drv_write_byte(struct spi_device *spi, u8 data) {
...
printk("Data from spi: %d\n", data);

// Måske skal der skrives værdien 8, ved rx_bux her, sådan at der i rx_buf ligger start.
...
ssize_t spi_drv_write(struct file *filep, const char __user *ubuf,
        zzzzAe              size_t count, loff_t *f_pos)
{
  ...
  if(copy_from_user(&value, ubuf, 1))
    return -EFAULT;
  ...
  */
  ...
  printk("number of devices:  %i\n", spi_devs_cnt);
  int err = spi_drv_write_byte(spi_devs[minor].spi, value);

printk(KERN_ALERT "wrote %i got err %i\n", value, err);
 ...
 */
 ...
 int spi_driver_read_byte(struct spi_device *spi, u8 *data) {
   struct spi_transfer t[1];
   struct spi_message m;
                      ...
                      size_t count, loff_t *f_pos)
 {
   ...
   */
   int err = spi_driver_read_byte(spi_devs[minor].spi, &result);
   printk(KERN_ALERT "Modtog %i fik fejl: %i\n", result, err);
